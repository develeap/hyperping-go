...
func TestClient_doRequest_InvalidURL(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "invalid base URL") && !strings.Contains(err.Error(), "failed to create request") {
...
func TestBuildHTTPRequest(t *testing.T) {
	...
	t.Run("User-Agent contains provider name", func(t *testing.T) {
		...
		if !strings.Contains(ua, "hyperping-go") {
			t.Errorf("User-Agent %q does not contain 'hyperping-go'", ua)
		}
	})
